...
/**
 * How ready this application is for AI-assisted development: is the Rapyd Admin
 * guideline in front of the coding agent, are the skills installed and current,
 * is Boost / MCP configured, and does every app module follow the conventions the
 * agent is taught (authorization, permissions, Limits, workflows, tests).
 *
 * Everything is read from the files of the application: no provider is called and
 * nothing is written. `rpd:ai:status` prints it, ai-module shows it as a page.
 */
class AiReadiness
{
    ...
    /**
     * @return array{tooling: array, context: array, modules: array, summary: array}
     */
    public function report(): array
    {
        ...
        $ok = count(array_filter($tooling, fn ($t) => $t['status'] === 'ok'));
        $protected = count(array_filter($modules, fn ($m) => $m['unprotected'] === []));
        ...
        return [
            ...
            'summary' => [
                'tooling_ok' => $ok,
                'tooling_total' => count($tooling),
                'modules_total' => count($modules),
                'modules_protected' => $protected,
                'ready' => $ok === count($tooling) && $protected === count($modules),
            ],
        ];
    }

    /**
     * One row per thing the coding agent needs: status ok | warn | missing, a detail
     * and, when not ok, the command that fixes it.
     *
     * @return list<array{key: string, label: string, status: string, detail: string, fix: ?string}>
     */
    public function tooling(): array
    {
        $rows = [];
        ...
        // who maintains the files: Boost (boost:update) or rpd:ai
        ...
        foreach ($this->packageSkills() as $name) {
            ...
            if (! is_dir($target)) {
                $rows[] = $this->row("skill:{$name}", "Skill {$name}", 'missing', 'not in .claude/skills', $refresh);
            } elseif ($this->sameFiles($this->skillsPath($name), $target)) {
                $rows[] = $this->row("skill:{$name}", "Skill {$name}", 'ok', '.claude/skills, current', null);
        ...
        $rows[] = $where === 'AGENTS.md' && ! $imports
            ? $this->row('claude_md', 'CLAUDE.md', 'warn', 'does not import AGENTS.md: Claude Code will not read the guideline', 'php artisan rpd:ai')
            : $this->row('claude_md', 'CLAUDE.md', $claude !== '' ? 'ok' : 'warn', $claude !== '' ? 'present' : 'absent (agents other than Claude Code read AGENTS.md)', $claude !== '' ? null : 'php artisan rpd:ai');
    ...
    /**
     * The application's own modules (app/Modules/*) against the conventions the agent
     * is taught: full-page components must authorize; permissions, Limits, workflows
