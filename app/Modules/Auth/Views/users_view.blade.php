    ...
    <div class="col-md-7">
        <x-rpd::card>
            <x-rpd::view title="User Detail">
              <x-slot name="buttons">
                <a href="{{ route_lang('auth.users') }}" class="btn btn-outline-primary">{{ __('List') }}</a>
                <a href="{{ route_lang('auth.users.edit',$user->id) }}" class="btn btn-outline-primary">{{ __('Edit') }}</a>
              </x-slot>
