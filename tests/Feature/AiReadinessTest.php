...
/**
 * AiReadiness reads a throwaway application: empty, after rpd:ai, with an older
 * guideline, with a module whose page does not authorize.
 */
class AiReadinessTest extends TestCase
{
    ...
    {
        return collect($report['tooling'])->firstWhere('key', $key);
    }

    public function test_an_empty_application_is_not_ready_and_every_row_names_its_fix()
    {
        $report = (new AiReadiness($this->appDir))->report();
    ...
    public function test_a_module_page_without_authorize_is_reported()
    {
        ...
        $this->assertSame(0, $report['summary']['modules_protected']);
    }

    public function test_the_command_prints_the_report_and_fails_until_ready()
    {
        $this->artisan('rpd:ai:status', ['--path' => $this->appDir])
            ->expectsOutputToContain('Rapyd Admin guideline')
            ->expectsOutputToContain('Not ready yet')
            ->assertFailed();

        $this->artisan('rpd:ai:status', ['--path' => $this->appDir, '--json' => true])->run();
        $this->artisan('rpd:ai', ['--path' => $this->appDir]);
        File::put("{$this->appDir}/.mcp.json", json_encode(['mcpServers' => ['laravel-boost' => []]]));

        $report = (new AiReadiness($this->appDir))->report();
        $notOk = collect($report['tooling'])->where('status', '!=', 'ok')->pluck('key')->all();
